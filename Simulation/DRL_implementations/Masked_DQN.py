"""
Masked Deep Q-Network (DQN) Implementation with Double DQN Support
This code is based on the DQN implementation in Pytorch:
 https://docs.pytorch.org/tutorials/intermediate/reinforcement_q_learning.html

This module implements a masked DQN agent for reinforcement learning environments with
action masking capabilities. Action masking allows the agent to respect dynamic action
constraints during both training and inference.

Classes:
    - ReplayMemory: Experience replay buffer for storing and sampling transitions
    - DQN: Neural network architecture for Q-value approximation
    - MaskedDQN: Standard DQN with action masking support
    - MaskedDoubleDQN: Double DQN variant with action masking for reduced overestimation

Key Features:
    1. Action Masking: Respects dynamic action constraints by masking invalid actions
       with large negative values (-1000) before action selection and Q-value computation

    2. Double DQN: Optional implementation that decouples action selection from Q-value
       estimation to reduce overestimation bias:
       - Policy network selects best action based on masked Q-values
       - Target network evaluates the Q-value of the selected action

    3. Experience Replay: Stores transitions (state, action, next_state, reward, mask)
       and samples mini-batches for training stability

    4. Soft Target Updates: Gradually updates target network using tau parameter for
       smoother learning

    5. Epsilon-Greedy Exploration: Exponentially decaying exploration rate with
       masked random action sampling when exploring

Training Process:
    - Collects experiences with epsilon-greedy policy respecting action masks
    - Stores transitions including action masks in replay memory
    - Samples random mini-batches and computes masked TD targets
    - Updates policy network using Smooth L1 (Huber) loss
    - Periodically updates target network via soft update

Action Masking Implementation:
    - Invalid actions are masked by setting Q-values to -1000 before argmax
    - Masks are stored in replay memory and applied during target computation
    - Random exploration samples only from valid actions using the mask
...
class MaskedDQN:
    ...
    def learn(self, training_steps: int = 200000, seed: int = None):
        ...
        for t in range(training_steps):
            action, _ = self.predict(state, _env.action_masks().astype(bool))
            obs, reward, terminated, truncated, info = _env.step(action.item())
            reward = torch.tensor([[reward]], dtype=torch.float32)
            mask = torch.tensor(_env.action_masks().astype(bool)).unsqueeze(0)
    ...
    def predict(self, observation, action_masks, deterministic=False,):
        ...
        :return:
        """
        mask = action_masks.astype(dtype=bool)
    ...
    def _evaluate_model(self):
        ...
        for t in range(10000):
            ...
            action, _ = self.predict(state, action_masks=_env.action_masks().astype(bool), deterministic=True)
